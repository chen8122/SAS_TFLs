libname octech '/home/u63467805/octech';
run;

data adeff_data0 (keep=subjectid visit aval paramcd chg1-chg3);
	set octech.adeff;
	if visit in ('Baseline', 'Month 12/15', 'Month 24', 'Month 36') and paramcd in ('AHI', 'ODI', 'T90');
run;

data adeff_data;
set adeff_data0;
	if visit = 'Month 12/15' then visit = 'month1';
	else if visit = 'Month 24' then visit = 'month2';
	else if visit = 'Month 36' then visit = 'month3';
run;

/* proc print data=adeff_data; */
/* run; */
...
/* proc print data=summaryStats_&avar; */
/* title "stats_&avar"; */
/* run; */
...
/* ods pdf file='/home/u63467805/octech/report.pdf'; */
/* proc print data=table_frame noobs label; */
/* 	title 'Table 2.1: Changes in PSG Outcomes over Time'; */
...
**** Report;
ods rtf file='/home/u63467805/octech/report.rtf';

proc report data=table_frame nowd;
	title 'Table 2.1: Changes in PSG Outcomes over Time';
	columns (Variable Baseline month1 month2 month3);
	define Variable / display 'Variable' style(column)={borderbottomcolor=black borderbottomwidth=1pt};
	define Baseline / display 'Baseline';
	define month1 / display 'Month 12-15';
	define month2 / display 'Month 24';
	define month3 / display 'Month 36';
run;
ods rtf close;
